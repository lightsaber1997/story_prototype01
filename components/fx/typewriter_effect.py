...
class TypewriterEffect(QObject):
    ...
    def update(self, new_full_text: str):
        ...
        # Calculate common prefix (already typed part)
        common_len = 0
        while (common_len < len(self._tokens) and
               common_len < len(new_tokens) and
               self._tokens[common_len] == new_tokens[common_len]):
            common_len += 1

        # Reset collected text and tokens to type
